            ...
            if(!strcmp(opcode,myoptab[chk].code))
            {
                ...
                if(operand[0]!='\0')
                {
                    for(chk=0;chk<symcount;chk++)
                        ...
                        {
                            _itoa(mysymtab[chk].addr,value,10);
                            foundoperand=1;
                        }
...
void chk_opcode()
{
    ...
    if(!found)
    {
        ...
        else if (!strcmp(opcode,"RESW"))locctr+=(3*myAtoi(operand));
        else if(!strcmp(opcode,"RESB"))locctr+=myAtoi(operand);
    }
}
